...
func (matrix *Matrix) EigenMatrix() *PolyMatrix {
	...
	for phaIdx := 0; phaIdx < phalanxSize; phaIdx++ {
		lambdaPoly := poly.ConstructPolyNode(1.0, 1).Poly(EigenPolyMatrixDefaultAES)
...
func (matrix *Matrix) DdLambda(k int) *poly.Poly {
	return matrix.constantFactors()[k]
}

func (matrix *Matrix) constantFactors() []*poly.Poly {
	return nil
}
...
func (matrix *Matrix) determinantFactors() []*poly.Poly {
	return nil
}
...
func (matrix *Matrix) convertToPolyMatrix() *PolyMatrix {
	...
	for rowIdx := 0; rowIdx < matrix.rowSize; rowIdx++ {
		for columnIdx := 0; columnIdx < matrix.columnSize; columnIdx++ {
			pm.set(rowIdx, columnIdx, poly.ConstructPolyNode(matrix.get(rowIdx, columnIdx), 0).Poly(EigenPolyMatrixDefaultAES))
		}
	}
	return pm
}
